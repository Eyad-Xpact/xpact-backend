...
// Mirror x position for RTL: x_rtl = W - x - elementWidth
...
function logo(slide, isArabic) {
  // Logo always top-left for LTR, top-right for RTL
...
async function buildProposal(data, outputPath) {
  ...
  // ── SLIDE 1: COVER ──
  {
    ...
    const titleLine1 = fa ? 'المقترح' : 'TECHNICAL';
    const titleLine2 = fa ? 'الفني'   : 'PROPOSAL';
    const tx = textX + 0.35;
    s.addText(titleLine1, { x:tx, y:1.2, w:4.8, h:0.45, fontSize:28, fontFace:f, bold:true, color:C.white, charSpacing:fa?0:4, margin:0, rtlMode:fa, align:al });
    s.addText(titleLine2, { x:tx, y:1.65, w:4.8, h:0.45, fontSize:28, fontFace:f, bold:true, color:C.mint, charSpacing:fa?0:4, margin:0, rtlMode:fa, align:al });
  ...
  // ── SLIDE 2: TABLE OF CONTENTS ──
  {
    ...
    // Dark panel: left for LTR, right for RTL
    const panelX = fa ? W - 3.2 : 0;
    const logoX  = fa ? panelX + 0.3 : 0.3;
    const titleX = fa ? panelX + 0.3 : 0.3;
    const lineX  = fa ? panelX + 0.3 : 0.3;
    ...
    s.addImage({ data: IMGS.logo, x:logoX, y:0.25, w:0.5, h:0.5 });
    s.addText(L.table_of_contents.replace(' ', '\n'), { x:titleX, y:2.2, w:2.6, h:0.9, fontSize:20, fontFace:f, bold:true, color:C.white, margin:0, rtlMode:fa, align:al });
    s.addShape('rect', { x:lineX, y:3.18, w:1.2, h:0.05, fill:{color:C.mint}, line:{color:C.mint} });
  ...
  // ── SLIDE 4: PROJECT MANAGEMENT ──
  {
    ...
    // For Arabic: reverse phase order so Pre-Event is on the right
    const orderedPhases = fa ? [...phases].reverse() : phases;

    orderedPhases.forEach((ph, i) => {
      const x = 0.55 + i * 3.15;
      // Middle card is highlighted — for Arabic the middle is still index 1
      const isMiddle = i === 1;
  ...
  // ── SLIDE 5: OBJECTIVES DETAIL ──
  {
    ...
    if (objIntro) {
      s.addText(objIntro.slice(0,200), { x:0.4, y:1.25, w:9.2, h:0.4, fontSize:10, fontFace:f, color:C.g2, margin:0, rtlMode:fa, align:al });
    }
  ...
  // ── SLIDE 7: OUR APPROACH ──
  {
    ...
    // For Arabic: step 01 starts from the RIGHT (index 0 = rightmost)
    ...
    diffs.forEach((d, i) => {
      let col = i % 2;
      if (fa) col = 1 - col; // mirror columns for Arabic
      const x = 0.4 + col * 4.85;
  ...
  // ── SLIDE 8: WHY XPACT ──
  {
    ...
    // Stats panel: right for LTR, left for RTL
  ...
  // ── SLIDE 9: ABOUT XPACT ──
  {
    const s = pres.addSlide();
    s.background = { color: C.dark };
    // Image: left for LTR, right for RTL
    ...
    stats.forEach((st, i) => {
      // For Arabic: reverse stat order right-to-left
      const idx = fa ? (2 - i) : i;
  ...
  // ── SLIDE 10: CONTACT ──
  {
    ...
    s.background = { color: C.dark };
    s.addShape('rect', { x:0, y:0, w:10, h:0.06, fill:{color:C.mint}, line:{color:C.mint} });
    // Logo and text: left for LTR, right for RTL
