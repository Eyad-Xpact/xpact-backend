...
if (args.length < 1) { console.log('Usage: node generate_proposal.js proposal_data.json'); process.exit(1); }
const data = JSON.parse(fs.readFileSync(args[0], 'utf8'));
const name = (data.event_name || 'Proposal').replace(/[^\w\u0600-\u06FF]/g, '_');
const outputPath = args[1] || path.join(path.dirname(args[0]), name + '_Proposal.pptx');
buildProposal(data, outputPath);
